...
def cumulative_error(errors, nbins=100000):
    ...
    values, base = np.histogram(
        errors, bins=nbins
    )  # values, base = np.histogram(1000*errors, bins=nbins)
...
def generating_cumulative_error_plots(
    method_error_fnames: list, method_identifiers: list, out_fname: str
):
    """
    ...
    for fname in method_error_fnames:
        method_errors.append(
            np.load(fname, allow_pickle=True, encoding="latin1").item()[
                "computed_distances"
            ]
        )
    ...
    for i in range(len(method_identifiers)):
        print(
            "%s - median: %f, mean: %f, std: %f"
            % (
                method_identifiers[i],
                np.median(np.hstack(method_errors[i])),
                np.mean(np.hstack(method_errors[i])),
                np.std(np.hstack(method_errors[i])),
            )
        )
