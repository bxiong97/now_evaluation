...
def MeshToScan(
    ...
    sampler, n_samples = construct_sampler(
        mesh_template_or_sampler, mesh_verts.size / 3
    )
...
def construct_sampler(sampler_or_template, num_mesh_verts):
    if isinstance(sampler_or_template, dict):
        ...
        sampler = sample_from_mesh(
            sampler_or_template, sample_type="uniformly-from-vertices", num_samples=1e5
        )

    n_samples = (
        sampler["point2sample"].shape[0] / 3
        if "point2sample" in sampler
        else num_mesh_verts
    )
...
class MeshDistanceSquared(Ch):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        if self.signed:
            result = sp.spdiags(
                self.direction, [0], self.direction.size, self.direction.size
            ).dot(result)
        return result
    ...
    def direction(self):
        ...
        fn = TriNormals(v=self.reference_verts, f=self.reference_faces).r.reshape(
            (-1, 3)
        )
        vn = VertNormals(
            f=self.reference_faces,
            num_verts=self.reference_verts.shape[0],
            v=self.reference_verts,
        ).r.reshape((-1, 3))
        ...
        nearest_normals[cl_vrt_idxs] = vn[
            self.reference_faces[nearest_tri, nearest_part]
        ]

        nearest_tri = self.nearest_tri[cl_edg_idxs]

        nearest_part = np.asarray(self.nearest_part[cl_edg_idxs] - 1, dtype=np.int64)
        nearest_normals[cl_edg_idxs] += vn[
            self.reference_faces[nearest_tri, nearest_part]
        ]

        nearest_part = np.asarray(
            np.mod(self.nearest_part[cl_edg_idxs], 3), dtype=np.int64
        )
        nearest_normals[cl_edg_idxs] += vn[
            self.reference_faces[nearest_tri, nearest_part]
        ]
    ...
    def on_changed(self, which):
        ...
        if not hasattr(self, "sampler") or self.sampler is None:
            ...
            sample_mesh = Mesh(
                v=self.sample_verts.r.reshape((-1, 3)), f=self.sample_faces
            )
            self.sampler = sample_from_mesh(
                sample_mesh, sample_type="uniformly-from-vertices", num_samples=1e5
            )
            which.add("sampler")
        ...
        if "reference_verts" in which or "reference_faces" in which:
            self.tree = _AabbTree(
                self.reference_verts.r.reshape((-1, 3)), self.reference_faces
            )
        ...
        if self.sampler["point2sample"] is not None:
            ...
            self.sample_points = self.ss_point2sample.dot(
                col(self.sample_verts.r)
            ).reshape(-1, 3)
        else:
...
class _AabbTree(object):
    ...
    def __setstate__(self, d):
        ...
        self.cpp_handle = spatialsearch.aabbtree_compute(
            self.vv.astype(np.float64).copy(order="C"),
