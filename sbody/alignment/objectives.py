...
def sample_from_mesh(
    mesh,
    sample_type="edge_midpoints",
    num_samples=10000,
    vertex_indices_to_sample=None,
    seed=0,
):
    ...
    if sample_type == "vertices":
        if vertex_indices_to_sample is None:
            sample_spec = {
                "point2sample": sy.sparse.eye(mesh.v.size, mesh.v.size)
            }  # @UndefinedVariable
        else:
        ...
        sample_ind = np.array(
            random.sample(
                range(mesh.v.shape[0]), int(min(num_samples, mesh.v.shape[0]))
            )
        )
        ...
        if sample_type == "edge-midpoints":
            tri = np.tile(
                array(range(0, mesh.f.size[0])).reshape(-1, 1), 1, 3
            ).flatten()
            ...
            bary = np.tile(
                [[0.5, 0.5, 0], [0.5, 0, 0.5], [0, 0.5, 0.5]], 1, mesh.f.shape[0]
            )
        ...
        sample_spec = {
            "area2weight": area2weight,
            "point2sample": point2sample,
            "tri": tri,
            "bary": bary,
        }
    return sample_spec
